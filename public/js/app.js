  ...
  sessions.forEach(session => {
    ...
    html += `
      <div class="history-item" data-session-id="${session.session_id}" style="cursor: pointer;">
        ...
        <div class="history-item-actions">
          <button class="btn-small" onclick="event.stopPropagation(); window.downloadSession('${session.session_id}', event)">Download</button>
...
async function downloadSession(sessionId, event) {
  ...
  markdown += `**Date:** ${new Date(firstExchange.timestamp).toLocaleDateString()}\n`;
  markdown += `**Conversation ID:** ${sessionId}\n\n`;
  markdown += `---\n\n`;
  ...
  markdown += `\n*Generated by Multi-Perspective AI*\n`;
  ...
  a.href = url;
  a.download = `MPAI_${dateStr}_${sessionTitle.replace(/[^a-z0-9]/gi, '_').substring(0, 30)}.md`;
